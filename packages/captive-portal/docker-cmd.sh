...
# docker image
#
IMAGE_NAME="node"  # eclipse-temurin, amazoncorretto
#IMAGE_VARIANT="alpine"        # alpine, debian

# tested combinations:
# maven java image           variant
#   3.9    8 eclipse-temurin 
#   3.9    8 eclipse-temurin alpine
#   3.9   11 eclipse-temurin 
#   3.9   11 eclipse-temurin alpine
#   3.9   17 eclipse-temurin 
#   3.9   17 eclipse-temurin alpine
#   3.9    8 amazoncorretto
#   3.9    8 amazoncorretto  debian 
#   3.9   11 amazoncorretto 
#   3.9   11 amazoncorretto  debian
#   3.9   17 amazoncorretto 
#   3.9   17 amazoncorretto  debian
#
#DOCKER_IMAGE="maven:${MAVEN_VERSION}-${IMAGE_NAME}-${JAVA_VERSION}${IMAGE_VARIANT+-${IMAGE_VARIANT}}"

DOCKER_IMAGE="${IMAGE_NAME}:21-slim"
...
CON_CMD="${CON_CMD} && su ${MY_USER_NAME} -c 'pwd && ls -la && cd captive-portal && pwd &&  pnpm install && npm run build && BROWSER=none npm run start'"
